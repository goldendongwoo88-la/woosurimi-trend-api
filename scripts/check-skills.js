...
for (const skill of ["fashion-review", "beauty-review"]) {
  const p = load(skill);
  console.log(`  ${skill}`);
  ok(/블로그 117개/.test(p), "  실측 근거가 들어갔다");
  ok(/두 자료가 겹치는 구간/.test(p), "  두 자료를 겹쳐서 잡았다고 되어 있다");
  ...
  ok(/협찬·제품 제공 글에는 넣지 않습니다/.test(p), "  협찬 글에는 링크 안 넣는다고 되어 있다");
  ok(/판정을 맨 위|판정만/.test(p), "  후기는 판정을 위에 두라고 되어 있다");
  ok(!/소제목 5~6개로 나눈다/.test(p), "  옛 '소제목 6개' 지침이 사라졌다");
  ok(/1370~2100자/.test(p), "  패션 글자수 범위가 맞다 (두 자료 겹치는 구간)");
  console.log("");
}
...
console.log("━━ 연예 스킬 (다른 값이어야 함) ━━\n");
{
  ...
  ok(/900~1700자/.test(p), "  연예는 글자수가 더 짧다 (900~1700)");
  ok(/20~90자/.test(p), "  연예는 사진 간격이 더 촘촘하다 (20~90)");
