...
async function plan({ title = "", body = "", genre = "", skillId = "" } = {}) {
  ...
  /**
   * ⚠️ 2026-09-01 사장님 지시: **"홈판에 상위노출 된 사람들과 비슷하게."**
   * 그래서 갈래 자동판별을 쓰지 않습니다.
   *
   * 함정이 있었습니다 — man_is_best는 뷰티 스킬이라 자동판별이 "뷰티 메이트"로
   * 떨어집니다. 뷰티 메이트 중앙값은 굵게 2.4로 **홈판의 1/4**입니다.
   * 그대로 두면 사장님이 원하신 것과 정반대로 강조가 줄어듭니다.
   *
   * 목표가 "홈판 노출"이므로 **홈판 실측을 기준으로 고정**합니다.
   * 다른 갈래로 쓰고 싶으면 genre를 직접 넘겨야 합니다(자동으로는 안 바뀝니다).
   */
  const useGenre = (genre && homefeed.MATE.groups[genre] && genre) || DEFAULT_GENRE;
  ...
  const prompt =
    ...
    `본문 (${chars}자):\n${text.slice(0, 6000)}\n\n` +
    `이 글에서 강조할 자리를 고르세요.\n\n` +
    `### 왜 하는지 — 이게 판단 기준입니다 (사장님 지시, 2026-09-01)\n` +
    `  1. **오래 머물게 한다.** 읽다가 나가지 않게 시선이 쉴 자리와 다음을 궁금하게 하는 자리를 만든다.\n` +
    `  2. **모바일에서 잘 읽히게 한다.** 거의 다 휴대폰으로 봅니다. 좁은 화면에서 눈이 걸리지 않아야 합니다.\n` +
    `개수를 채우는 게 목적이 아닙니다. 위 둘에 도움이 안 되는 강조는 빼세요.\n` +
    `특히 **한 화면(약 300자)에 강조가 서너 개를 넘으면** 오히려 눈이 피로해서 나갑니다.\n\n` +
    `### 기준: 네이버 홈판 상위노출 블로그 실측 (${homefeed.MATE.measuredAt})\n` +
    `홈판으로 잘 되는 블로그 8곳·40편을 세어본 값입니다. 그들처럼 씁니다.\n` +
    `  1,000자당 — 굵게 ${per1000.bold} · 글자색 ${per1000.color} · 밑줄 ${per1000.underline} · 배경색 ${per1000.highlight}\n` +
    `  이 글(${chars}자) 목표 — 굵게 ${targets.bold}개 · 밑줄 ${targets.underline}개 · 배경색 ${targets.highlight}개 · 글자색 ${targets.color}개\n\n` +
    `**개수를 되도록 채우세요.** 강조가 촘촘한 글을 원합니다.\n\n` +
    `### 홈판 사람들이 강조를 거는 방식 (이대로 따라 하세요)\n` +
    `1. **굵게가 주력입니다.** 전체 강조의 3분의 2가 굵게입니다. 색이 아니라 굵게로 리듬을 만듭니다.\n` +
    `2. **색은 아껴 씁니다.** 굵게의 3분의 1도 안 됩니다. 색을 남발하면 아마추어로 보입니다.\n` +
    `   색은 **가격·수치·날짜**처럼 눈으로 찾아야 하는 것에만 겁니다.\n` +
    `3. **배경색은 글 전체에 한두 번.** 딱 하나 기억시킬 문장에만.\n` +
    `4. 한 문단에 강조가 두 개를 넘지 않게 흩뿌립니다. 몰아 칠하면 안 읽힙니다.\n` +
    `5. **문장 전체가 아니라 핵심 낱말·구절**에 겁니다. 긴 문장을 통째로 굵게 하면 강조가 사라집니다.\n` +
    `6. 거는 자리: 결론 문장 · 숫자/가격 · 제품명 · 비교 지점 · 남들과 다른 주장.\n` +
    `   안 거는 자리: 인사말 · 접속사 · 당연한 설명 · 이미 소제목인 줄.\n\n` +
