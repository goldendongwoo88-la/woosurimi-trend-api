 ...
 * (scripts/measure-emphasis.js — 두 블로그 각 10편)
 *
 *   1,000자당          잘 되는 쪽(nidle_831)   사장님(man_is_best)
 *   굵게                     12.8                  7.6
 *   밑줄                      9.1                  0      ← 아예 안 씀
 *   배경색                    6.7                  0      ← 아예 안 씀
 *   글자색                    5.3                 35.6     ← 7배
 *   소제목(인용구) 글 한 편당    12개                  2개
 *
 * 정반대입니다. 잘 되는 쪽은 네 가지를 고루 쓰는데, 사장님은 **글자색만**
 * 잔뜩 쓰고 밑줄·배경색은 한 번도 안 씁니다.
 * 글자색을 1,000자에 35번 쓰면 그건 강조가 아니라 그냥 색깔 글입니다.
 * 다 강조하면 아무것도 강조가 안 됩니다.
 *
...
/** 실측값 — 화면에서 근거로 보여줄 때도 씁니다. */
const MEASURED = {
  source: "두 블로그 각 10편 (2026-08-27)",
  winner: { id: "nidle_831", daily: 74094, bold: 12.8, underline: 9.1, highlight: 6.7, color: 5.3, quotes: 12 },
  baseline: { id: "man_is_best", daily: 21018, bold: 7.6, underline: 0, highlight: 0, color: 35.6, quotes: 2 },
};

/**
 * 1,000자당 몇 개를 넣을지. 잘 되는 쪽 값을 그대로 씁니다.
 *
 * ⚠️ 다만 조금 낮춰 잡았습니다. 실측은 HTML 태그를 센 값이라, 한 군데에
 * 굵게+색을 같이 쓰면 두 번으로 세어집니다. 실제 "강조한 자리"는 그보다 적습니다.
 */
const PER_1000 = { bold: 9, underline: 6, highlight: 4, color: 3 };

/** 한 번에 너무 많이 넣지 않게. 아무리 긴 글이어도 이 이상은 안 합니다. */
const HARD_CAP = { bold: 30, underline: 20, highlight: 14, color: 10 };
...
실제로 잘 되는 블로그를 세어보니 1,000자에 굵게 13번, 밑줄 9번,
배경색 7번, 글자색 5번쯤입니다. 그 선을 넘지 마세요.
특히 배경색은 제일 세게 튀니까 아껴 쓰세요.
