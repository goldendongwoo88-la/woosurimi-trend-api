...
/**
 * 카테고리 트렌드를 얼마나 오래 재사용할지.
 *
 * ⚠️ 10분이었습니다. 그래서 **네이버 오픈API 일별 한도를 다 태웠습니다.**
 * 실제로 429가 떴습니다: "일별 사용량 한도가 초과하였습니다."
 * 그날 문서 수·기회 점수가 전부 빈칸으로 나왔습니다.
 *
 * 계산해보니 이랬습니다:
 *   카테고리 8개 × 후보 60개 × 2회(문서수+오늘글수) × 하루 144번 = 138,240회
 *
 * ⚠️ 게다가 **캐시 유효기간과 예열 주기가 똑같이 10분**이었습니다.
 * 예열하러 갈 때마다 캐시가 방금 만료돼 있어서 매번 새로 받았습니다.
 * 캐시를 둔 의미가 아예 없었습니다.
 *
 * 6시간으로 늘립니다. 트렌드 키워드가 10분마다 바뀌지는 않습니다.
 * 그리고 예열 주기는 캐시보다 **짧게** 둡니다(아래 참고) — 그래야 손님이
 * 왔을 때 캐시가 살아 있습니다.
 */
const BLOG_TOPICS_TTL_MS = Number(process.env.BLOG_TOPICS_TTL_MS || 6 * 60 * 60 * 1000);
...
/**
 * ⚠️ 예전에는 캐시 유효기간(분)을 그대로 크론 식에 넣었습니다.
 * 10분일 땐 잘 돌았습니다. 그런데 유효기간을 6시간으로 늘리면 분 칸에 360이 들어갑니다.
 *
 * **터지지는 않습니다.** 크론은 유효하다고 받아줍니다. 그런데 분 칸은 0~59라서
 * 360 간격으로 맞는 분은 0 하나뿐입니다. 즉 6시간마다가 아니라 **매시 0분**,
 * 한 시간마다 돕니다. 의도한 것보다 6배 자주 도는데 아무도 모릅니다.
 * (직접 재봤습니다 — cron.validate 는 true 를 주고, 매칭되는 분은 [0] 뿐입니다)
 *
 * 조용히 틀리는 게 터지는 것보다 나쁩니다. 분으로 계산하는 방식을 버리고
 * 시간 단위로 정직하게 적습니다.
 *
 * ⚠️ 예열은 캐시보다 **자주** 돌아야 합니다(6시간 캐시 → 3시간마다 예열).
 * 같은 주기로 두면 손님이 왔을 때 캐시가 막 만료돼 있어서 그 자리에서 기다리십니다.
 * 예전이 딱 그 상태였습니다(둘 다 10분) — 캐시가 있으나 마나였습니다.
 */
const WARM_HOURS = Math.max(1, Math.min(12, Math.round(BLOG_TOPICS_TTL_MS / 3600000 / 2) || 1));
cron.schedule(`0 */${WARM_HOURS} * * *`, warmBlogTopicsCache, { timezone: "Asia/Seoul" });
console.log(`[예열] 카테고리 트렌드를 ${WARM_HOURS}시간마다 미리 받아둡니다 (캐시 ${Math.round(BLOG_TOPICS_TTL_MS / 3600000)}시간).`);
